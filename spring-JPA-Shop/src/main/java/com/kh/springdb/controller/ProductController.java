...
/*
 @GetMapping("/update/{id}")
	...
	예외 값을 처리하는 방법 : orElse를 이용해서 대체값을 제공하거나 페이지 이동처리를 할 수 있음 (ex: error.html)
	이외에 orElseGet 대체값을 생성하는 함수를 제공
		orElseThrow
	product.ifPresent(value -> model.addAttribute("product", value));
	ifPresent : Optional 객체 안에 값이 존재할 경우 람다식 표현을 실행하기 위한 메서드
				Optional 객체 안에 값이 존재할 경우 실행할 것임, value값이 존재하면 모델에 product 변수명을 사용해서 product안에 value 값을 추가할 것
	 			추가된 product는 html 템플릿안에서 product를 thymeLeaf를 통해 호출해서 value값을 사용할 수 있음
	 			
	 			
	람다식 : 간결하게 함수를 표현하는 방법으로 간단하게 표현할 때 사용
